...
Patch 0007 adds `scripts/clean-dist.ts` and changes `bun run build` to run `clean:dist` first. This prevents stale deleted hook or command artifacts from surviving in `dist/` after rebuild.
